...
// Sonnet 4.6 держит объём тонких правил промпта (двухходовка, «2 факта→контакт»,
// маркеры, §9-запреты), которые Haiku ронял. temperature 0.5 на Sonnet 4.6 валидна
// (sampling params убраны только на Opus 4.8/4.7 и Fable 5). System-промпт кэшируется,
// ответы ≤1024 токенов → абсолютная стоимость мала при цене лида в сотни ₽.
const MODEL = "claude-sonnet-4-6";
// 1024 токена = ~680 слов. 500 обрезало развёрнутые ответы с реальными
// кейсами (особенно §2 в knowledge/05-возражения-про-конкурентов.md —
// рассказ про детские товары + перечисление 5 паттернов WB). Инструкция в
// промпте про 3-4 абзаца — мягкая, Анастасия может уйти за лимит когда
// кейс реально требует объёма. Лучше дать запас токенов, чем рвать ответ.
...
const FALLBACK_AFTER_LEAD =
  "Спасибо! Передала ваш контакт менеджеру. Он напишет вам первым в течение ~15 минут.";
